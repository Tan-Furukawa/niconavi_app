...
def generate_code_b(source_code: str, template_code: str) -> str:
    ...
    # テンプレート内の marker の下に生成コードを挿入
    marker = "# {{{from make stores}}}"
...
def main() -> None:
    ...
    with open(base_dir / "template.py", "r", encoding="utf-8") as f:
    ...
    with open(package_dir / "stores.py", "w", encoding="utf-8") as f:
        f.write(auto_msg + generated_code)
    print(
        f"State クラス、変換関数、save_in_～ 関数群を {package_dir / 'stores.py'} に出力しました。"
    )
